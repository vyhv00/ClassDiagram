...
public class ClassFactory {
    ...
    public static ClassLikeElement createClass(ClassTree element) {
        ...
        if (element.getKind().equals(Tree.Kind.ENUM)) {
            return new EnumElement(element.getSimpleName(), element.getModifiers().getFlags());
        }
        if (element.getKind().equals(Tree.Kind.RECORD)) {
            return new RecordElement(element.getSimpleName(), element.getModifiers().getFlags());
        }
        if (element.getKind().equals(Tree.Kind.CLASS)) {
